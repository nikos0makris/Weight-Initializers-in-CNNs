...
def lenet(initializer):
    ...
    model.add(Conv2D(filters=6, kernel_size=5, kernel_initializer=initializer, activation=tanh, input_shape=(32, 32, 3)))
    # 2x2 pooling
...
for _ in range(5):
    ...
    ax1.set_title('Accuracy on LeNet-C100 with Different Weight Initializers')
    ax2.set_xlabel('Epochs')
    ax2.set_ylabel('Loss')
    ax2.set_title('Loss on LeNet-C100 with Different Weight Initializers')
    plt.legend()
    ...
    # Save the plot
    plt.savefig(f'lenet_c100_{_+1}.png', dpi='figure')
    plt.close()
