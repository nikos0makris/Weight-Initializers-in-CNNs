...
def lenet(initializer):
    ...
    model.add(Conv2D(filters=96, kernel_size=5, kernel_initializer=initializer, activation=tanh, input_shape=(32, 32, 3)))
    # 2x2 pooling
    model.add(AveragePooling2D(pool_size=2, strides=2))
    # 5x5 kernel size, with 1 stride and 16 featured maps
    model.add(Conv2D(filters=128, kernel_size=5, kernel_initializer=initializer, activation=tanh))
    # 2x2 pooling
    model.add(AveragePooling2D(pool_size=2, strides=2))
    ...
    # Compile the model
    optimizer = Adam(learning_rate=0.000111)
...
for _ in range(5):
    ...
    for initializer in initializers_dict:
        ...
        # Train the models
        print()
        print(f'Training the LeNet-C100 model with a {initializer} weight initializer:')
    ...
    ax1.set_title('Accuracy on LeNet-C100 with GloU and Best Hyperparameters')
    ax2.set_xlabel('Epochs')
    ax2.set_ylabel('Loss')
    ax2.set_title('Loss on LeNet-C100 with GloU and Best Hyperparameters')
    plt.legend()
    ...
    # Save the plot
    plt.savefig(f'opt1_{_+1}.png', dpi='figure')
    plt.close()
