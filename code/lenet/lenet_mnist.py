...
for _ in range(5):
    ...
    for initializer in initializers_dict:
        ...
        # Train the models
        print()
        print(f'Training the LeNet-MNIST model with a {initializer} weight initializer:')
    ...
    ax1.set_title('Accuracy on LeNet-MNIST with Different Weight Initializers')
    ax2.set_xlabel('Epochs')
    ax2.set_ylabel('Loss')
    ax2.set_title('Loss on LeNet-MNIST with Different Weight Initializers')
    plt.legend()
    ...
    # Save the plot
    plt.savefig(f'lenet_mnist_{_+1}.png', dpi='figure')
    plt.close()
