...
def modnet2(initializer):
    # Sequential model
    model = Sequential()
    # 1st Convolutional Layer: (3x3 kernel size), with 1 stride and 32 filters
    model.add(Conv2D(filters=128, kernel_size=3, kernel_initializer=initializer, activation=relu, padding="same", input_shape=(32, 32, 3)))
    # 2nd Convolutional Layer: (3x3 kernel size), with 1 stride and 32 filters
    ...
    # 3rd Convolutional Layer: (3x3 kernel size), with 1 stride and 64 filters
    model.add(Conv2D(filters=256, kernel_size=3, kernel_initializer=initializer, activation=relu, padding="same"))
    # 4th Convolutional Layer: (3x3 kernel size), with 1 stride and 64 filters
    model.add(Conv2D(filters=192, kernel_size=3, kernel_initializer=initializer, activation=relu))
    # 2x2 Max Pooling
    model.add(MaxPooling2D(pool_size=2, strides=2))
    # Dropout regularization of 0.5
    model.add(Dropout(0.5))
    ...
    # Compile the model
    optimizer = SGD(learning_rate=0.0037, momentum=0.9)
...
for _ in range(5):
    ...
    for initializer in initializers_dict:
        ...
        # Train the models
        print()
        print(f'Training the ModNet2 model with a {initializer} weight initializer:')
    ...
    ax1.set_title('Accuracy on ModNet2 with LeN and Optimal Hyperparameters')
    ax2.set_xlabel('Epochs')
    ax2.set_ylabel('Loss')
    ax2.set_title('Loss on ModNet2 with LeN and Optimal Hyperparameters')
    plt.legend()
    ...
    # Save the plot
    plt.savefig(f'opt3_{_+1}.png', dpi='figure')
    plt.close()
