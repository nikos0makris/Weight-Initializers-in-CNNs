...
def modnet1(initializer):
    # Sequential model
    model = Sequential()
    # 1st Convolutional Layer: (3x3 kernel size), with 1 stride and 32 filters
    model.add(Conv2D(filters=32, kernel_size=3, strides=1, kernel_initializer=initializer, activation=relu, input_shape=(28, 28, 1)))
    # 2x2 Max Pooling
    model.add(MaxPooling2D(pool_size=2, strides=2))
    # 2nd Convolutional Layer: (3x3 kernel size), with 1 stride and 64 filters
    model.add(Conv2D(filters=64, kernel_size=3, strides=1, kernel_initializer=initializer, activation=relu))
    # 2x2 Max Pooling
    model.add(MaxPooling2D(pool_size=2, strides=2))
    # 3rd Convolutional Layer: (3x3 kernel size), with 1 stride and 128 filters
    model.add(Conv2D(filters=128, kernel_size=3, strides=1, kernel_initializer=initializer, activation=relu))
    # 2x2 Max Pooling
    model.add(MaxPooling2D(pool_size=2, strides=2))
    # Reshape to one-dimensional tensor
    model.add(Flatten())
    # 1st Dense Layer: 256 neurons
    model.add(Dense(256, activation=relu))
    # Dropout regularization of 0.5
    model.add(Dropout(0.5))
    # 2nd Dense Layer: 128 neurons
    model.add(Dense(128, activation=relu))
    # Dropout regularization of 0.5
    model.add(Dropout(0.5))
    # Output Layer: 10 neurons for the FMNIST dataset, with softmax activation
...
for _ in range(4):
    ...
    for initializer in initializers_dict:
        ...
        # Train the models
        print()
        print(f'Training the ModNet1-FMNIST model with a {initializer} weight initializer:')
    ...
    ax1.set_title('Accuracy on ModNet1-FMNIST with Different Weight Initializers')
    ax2.set_xlabel('Epochs')
    ax2.set_ylabel('Loss')
    ax2.set_title('Loss on ModNet1-FMNIST with Different Weight Initializers')
    plt.legend()
    ...
    # Save the plot
    plt.savefig(f'mod1_fmnist_{_+1}.png', dpi='figure')
    plt.close()
