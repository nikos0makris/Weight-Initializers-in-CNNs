...
def modnet1(initializer):
    ...
    model.add(Conv2D(filters=32, kernel_size=3, strides=1, kernel_initializer=initializer, activation=relu, input_shape=(32, 32, 3)))
    # 2x2 Max Pooling
    ...
    # Dropout regularization of 0.5
    model.add(Dropout(0.5))
    # Output Layer: 10 neurons for the MNIST  dataset, with softmax activation
...
for _ in range(1):
    ...
    for initializer in initializers_dict:
        ...
        # Train the models
        print()
        print(f'Training the ModNet1-MNIST model with a {initializer} weight initializer:')
    ...
    ax1.set_title('Accuracy on ModNet1-MNIST with Different Weight Initializers')
    ax2.set_xlabel('Epochs')
    ax2.set_ylabel('Loss')
    ax2.set_title('Loss on ModNet1-MNIST with Different Weight Initializers')
    plt.legend()
    ...
    # Save the plot
    plt.savefig(f'mod1_mnist_{_+1}.png', dpi='figure')
    plt.close()
