...
def modnet1(initializer):
    ...
    model.add(Conv2D(filters=64, kernel_size=3, strides=1, kernel_initializer=initializer, activation=relu, input_shape=(32, 32, 3)))
    # 2x2 Max Pooling
    ...
    model.add(Conv2D(filters=160, kernel_size=3, strides=1, kernel_initializer=initializer, activation=relu))
    # 2x2 Max Pooling
    model.add(MaxPooling2D(pool_size=2, strides=2))
    ...
    # Compile the model
    optimizer = SGD(learning_rate=0.000426, momentum=0.9)
...
for _ in range(5):
    ...
    for initializer in initializers_dict:
        ...
        # Train the models
        print()
        print(f'Training the ModNet1-Cifar100 model with a {initializer} weight initializer:')
    ...
    ax1.set_title('Accuracy on ModNet1-C100 with LeU and Optimal HyperParameters')
    ax2.set_xlabel('Epochs')
    ax2.set_ylabel('Loss')
    ax2.set_title('Loss on ModNet1-C100 with LeU and Optimal HyperParameters')
    plt.legend()
    ...
    # Save the plot
    plt.savefig(f'opt2_{_+1}.png', dpi='figure')
    plt.close()
