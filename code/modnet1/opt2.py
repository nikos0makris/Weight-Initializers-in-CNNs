...
def modnet1(filters, learning_rate):
    # Sequential model
    model = Sequential()
    model.add(Conv2D(filters=filters[0], kernel_size=3, strides=1, kernel_initializer='lecun_uniform', activation=relu, input_shape=(32, 32, 3)))
    # 2x2 Max Pooling
    model.add(MaxPooling2D(pool_size=2, strides=2))
    model.add(Conv2D(filters=filters[1], kernel_size=3, strides=1, kernel_initializer='lecun_uniform', activation=relu))
    # 2x2 Max Pooling
    model.add(MaxPooling2D(pool_size=2, strides=2))
    model.add(Conv2D(filters=filters[2], kernel_size=3, strides=1, kernel_initializer='lecun_uniform', activation=relu))
    # 2x2 Max Pooling
    model.add(MaxPooling2D(pool_size=2, strides=2))
...
def objective(trial):
    filters = [
        ...
        trial.suggest_int('filter3', 64, 256, step=32)
    ]
    learning_rate = trial.suggest_float('learning_rate', 1e-5, 1e-1, log=True)

    model = modnet1(filters, learning_rate)
